...
class YourFiles extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      percent: 0,
      feedback:
        "This Key will be used to encrypt your file, you can choose different keys for different files",
    };
  }
  ...
  //   decodeTransaction = async (transactions) => {
  //     const decodedTransactions = [];
  //     for (let i = 0; i < transactions.length; i++) {
  //       const responseDecode = await fetch(
  //         `https://api.covalenthq.com/v1/97/transaction_v2/${transactions[i].tx_hash}/?key=ckey_58f5096215a5431c9235c89345c`
  //       );
  //       const result = await responseDecode.json();
  //       const data = result.data;
  //       decodedTransactions.push(data);
  //     }
  ...
  componentDidMount = async () => {
    ...
    const response = await fetch(
      "https://api.covalenthq.com/v1/97/address/0x9c647e07e1d7feab9b53fdfa12007b5ebfcfa8b7/transactions_v2/?quote-currency=USD&format=JSON&block-signed-at-asc=false&no-logs=false&key=ckey_58f5096215a5431c9235c89345c"
    );
      ...
      reader.onload = () => {
        ...
        ipfs.files.add(Buffer(encrypted), (err, result) => {
          if (err) {
            ...
            contract.methods
              .addMyfile(result[0].hash, file.type, file.name)
              .send({ from: accounts[0], gas: 3000000 })
              .then((receipt) => {
          ...
          return (
            <div>
              <Button
                ...
                className="buttons"
                type="primary"
                onClick={() => this.handleDownloadButton(record)}
              >
                Download
              </Button>
              <Button
                ...
                className="buttons"
                type="primary"
                onClick={() => this.handleShareButton(record)}
              >
                Share
              </Button>
            </div>
      ...
      <div>
        ...
        >
          ...
          <div className="keyInsWrapper">
            <Text strong className="keyIns">
              The user at other end, will require key of this file to see its
              content.
            </Text>
            <ul className="keyInsL">
              <li>You can pass down the key, on yourself.</li>
              <li>
                Or you can use our <Text code>Key exchange</Text> service, which
                uses Deffi- Hellman Key exchange for transfer of key.
              </li>
            </ul>
          </div>
        </Modal>
        ...
        >
          ...
          <Progress
            ...
            className="feedback"
          />{" "}
          &nbsp; &nbsp; &nbsp;<Text strong>{this.state.feedback}</Text>
        </Modal>
